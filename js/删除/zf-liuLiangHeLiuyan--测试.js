$(function(){
	...
	function liuLiangHeLiuyan() {
		...
		let zfLoad = {
			visitDomain: window.location.hostname,  // 用户访问的域名 visit_domain
			visitPage: window.location.pathname,  // 用户访问的页面 visit_page
			visitPagePosition: 'footer',  // 页面位置 visit_page_position
			visitUrl: window.location.href, // 用户访问的完整链接 visit_url
			srcUrl: document.referrer,  // 来源链接地址	src_url
			srcDomain: '',  // 来源网站域名：搜索引擎或外链平台
			
			srcPlatform: navigator.platform,  // 来源平台 src_platform
			userLanguage: navigator.language || navigator.userLanguage,  // 用户的语言设置
			let userLanguages: navigator.languages.join(', '),  // 用户的语言偏好列表
			
			srcEngine :'',  // 来源搜素引擎 src_engine
			srcDevice: '',  // 来源设备 src_device
			srcOs: '', // 来源系统 src_os
			
			userAgentJs: navigator.userAgent,
			userAgent: userAgentJs.toLowerCase(),
			srcBrowser: '',  // 来源浏览器类型 src_browser
		}
		
		let formIsValid = true;
		
		let srcUrl = zfLoad.srcUrl;
		if (srcUrl) {
			const newSrcUrl = new URL(srcUrl);
			zfLoad.srcDomain = newSrcUrl.hostname;
			if (srcUrl.includes('google.com')) {
				zfLoad.srcEngine = 'Google';
			} else if (srcUrl.includes('baidu.com')) {
				zfLoad.srcEngine = 'Baidu';
			} else if (srcUrl.includes('bing.com')) {
				zfLoad.srcEngine = 'Bing';
			}
		}
		
		let userAgent = zfLoad.userAgent;
		if (/mobile|android|iphone|ipad/i.test(userAgent)) {
			zfLoad.srcDevice = 'Mobile';
		} else {
			zfLoad.srcDevice = 'Desktop';
		}
		
		if (/win/i.test(userAgent)) {
			zfLoad.srcOs = 'Windows';
		} else if (/mac/i.test(userAgent)) {
			zfLoad.srcOs = 'MacOS';
		} else if (/linux/i.test(userAgent)) {
			zfLoad.srcOs = 'Linux';
		} else if (/android/i.test(userAgent)) {
			zfLoad.srcOs = 'Android';
		} else if (/iphone|ipad|ipod/i.test(userAgent)) {
			zfLoad.srcOs = 'iOS';
		}
		
		if (/chrome/i.test(userAgent) && !/edge/i.test(userAgent)) {
			zfLoad.srcBrowser = 'Chrome';
		} else if (/firefox/i.test(userAgent)) {
			zfLoad.srcBrowser = 'Firefox';
		} else if (/safari/i.test(userAgent) && !/chrome/i.test(userAgent)) {
			zfLoad.srcBrowser = 'Safari';
		} else if (/edge/i.test(userAgent)) {
			zfLoad.srcBrowser = 'Edge';
		} else if (/opr\//i.test(userAgent)) {
			zfLoad.srcBrowser = 'Opera';
		} else if (/msie|trident/i.test(userAgent)) {
			zfLoad.srcBrowser = 'Internet Explorer';
		}
		...
		function zfLiuLiangSubmit() {
			$.ajax({
				...
				success: function(s){ // 请求成功要执行的代码 // 参数：是从服务器返回的数据
					zfLinkChek = s.t;
					zfFooterCheckTip.attr('zf-chek-data', zfLinkChek);
				},
				error: function(e){}
			});
		}
		...
		function zfSubErrorOpen(ef, code, time = 0) {
			...
			let t = ZF_ERR_ARR?.[ef]?.[code]?.t ?? ZF_ERR_TIP_TITLE;
